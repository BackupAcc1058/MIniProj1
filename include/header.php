    ...
    echo    "<h1 class='header-title'> $pageTitle </h1>";
            if ($_SESSION) {
                if ($_SESSION['role'] == "perusahaan") {
                    $aquery = "SELECT * FROM perusahaan WHERE id_user = ?";
                    ...
                    echo "<h2>Nama Perusahaan: ".$auser['nama_perusahaan']."</h2>";
                } elseif ($_SESSION['role'] == "pencari_kerja") {
                    $aquery = "SELECT * FROM pencari_kerja WHERE id_user = ?";
                    $astmt = $conn->prepare($aquery);
                    $astmt->bind_param("i", $_SESSION['user_id']);
                    $astmt->execute();
                    $aresult = $astmt->get_result();
                    ...
                    echo "<h2>User: ".$auser['nama_lengkap']."</h2>";
                }
            }
    echo    "<nav>";
